...
return xs} ) .call(this,e.flatten_all( ';' ) ) :e.resolved_data() === '=' ?define(e) :e.resolved_data() === '@js' ?js_evaluate(e) :e.resolved_data() === '=@js' ?js_define(e) :e.resolved_data() === '/-' ?evaluate(rewrite(e) ) :cc(e) } ,js_macroexpand=$( 'js_all' ) ,empty=new c.parse.syntax(c.parse.intern( '@' ) ) ,cc=function(x) {;
return( ( ( (options) || (defaults) ) ) ) .cc(x) } ,rewrite=function(t) {;
return c.toplevel.extend(t[1] .flatten_all( ';' ) ) (t[0] ) } ,js_evaluate=function(t) {;
return cc( ( ($.compile(js_macroexpand(t[0] .as_js() ) ,c.environment() ) ) || (empty) ) ) } ,js_define=function(equation) {;
