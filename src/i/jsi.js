...
return this.resolved_data() === '@' ?$.empty:this.resolved_data() === '#' ?new $.syntax( '()' ,this[0] .as_js() ,this[1] .as_js() ) :represents_a_slice(this) ?new $.syntax( '[]' ,this[0] .as_js() ,this[1] [0] .as_js() ) :this.length===0?new $.syntax(mh_decode_literal(this.resolved_data() ) ) :new $.syntax(mh_decode(this.resolved_data() ) , (function(xs) {var x,x0,xi,xl,xr;
...
return/\/./ .test(s) ? '"' +s.replace( /"/g , '\\"' ) + '"r' :s.replace( /^\$/ , '@$' ) } ,mh_encode=function(d) {var s=d.toString() ;
return/^[()\[\]{}\.,;:]/ .test(s) ?s: '/' +s} ,mh_decode_literal=function(d) {var s=d.toString() ;
return/^".*"r$/ .test(s) ?s.replace( /^"\// , '/' ) .replace( /\\"/g , '"' ) .replace( /"r$/ , '' ) :s.replace( /^@\$/ , '$' ) } ,mh_decode=function(d) {var s=d.toString() ;
return s.replace( /^\/(.)/ , '$1' ) } ;
