...
return ft|| (this) .map(function(_) {return(_) .rmap(f) } ) } ,as_caterwaul:function() {;
return new $.syntax(this.resolved_data() .replace( /^@dollar/ , '$' ) .replace( /^\// , '' ) , (function(xs) {var x,x0,xi,xl,xr;
for(var xr=new xs.constructor() ,xi=0,xl=xs.length,x0,_y;
xi<xl;
 ++xi)x=xs[xi] , (_y= (x.resolved_data() !== '_' &&x.as_caterwaul() ) ) &&xr.push(_y) ;
return xr} ) .call(this,Array.prototype.slice.call( (this) ) ) ) } ,flatten:function(op) {;
